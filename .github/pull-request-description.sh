...
# extract changelogs
CHANGELOGS_1=$(git diff HEAD --unified=0 --no-prefix -- '*CHANGELOG.md' | # get changes from CHANGELOG files
...
VERSIONS_1=$(git diff HEAD --unified=0 --no-prefix --relative='packages' '*package.json' | # get changes from package.json files
...
BODY=$(get_pull_request_body "$PULL_REQUEST" "$CHANGELOGS_1" "$VERSIONS_1")

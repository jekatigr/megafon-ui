...
function generate_pull_request_body () {
    ...
    local pull_request_description=$(echo "${pull_request}" |
...
# 1. получить массив реквестов, где либо нет autogenerated коммента, либо head.sha != хэшу из body
# 2. проитерироваться по массиву:
# 2.1 чекаутнуть head ветки из реквеста
# 2.2 обновить описание реквеста

#readarray -t arr < <(jq -rc '.[] | {title}' <<< "$PULL_REQUESTS")

for k in $(jq '. | keys | .[]' <<< "$PULL_REQUESTS"); do
    pull_request=$(jq -r ".[$k]" <<< "$PULL_REQUESTS");
    sha=$(jq -r ".head.sha" <<< "$pull_request");
    body=$(jq -r ".body" <<< "$pull_request" | sed -E ':a;N;$!ba;s/\r{0,1}\n/\\n/g' | sed 's/"/\\"/g');
